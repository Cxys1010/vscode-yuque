...
语雀原生高亮块**可以通过 API 创建**！使用 `:::` 语法（与网页编辑器一致）：

```markdown
:::info
这是 **info** 信息提示块的内容。
支持段落、列表、代码等所有 Markdown 语法。
:::
...
`/fl2` `/fl3` `/fl4` 语法**无法通过 API 创建**，但你可以用 HTML 表格模拟：
...
- **折叠块**（可展开/收起的内容区域）—— HTML `<details>` 标签会被移除
- **分栏布局**（真正的多栏，而非表格模拟）—— `/fl2` 语法不生效
- **特色卡片**（文件卡、看板、日历、投票等）
